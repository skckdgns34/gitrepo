...
<div class="input-group">
		<select name='search_type'>
			<option value='BOOK_NO'<c:if test="${search_type == 'BOOK_NO'}">selected</c:if>>책 번호</option>
			<option value='TITLE' <c:if test="${search_type == 'TITLE'}">selected</c:if>>책 이름</option>
			<option value='COMPANY_NAME'<c:if test="${search_type == 'COMPANY_NAME'}">selected</c:if>>출판사 이름</option>
			<option value='WRITER'<c:if test="${search_type == 'WRITER'}">selected</c:if>>작가 이름</option>
